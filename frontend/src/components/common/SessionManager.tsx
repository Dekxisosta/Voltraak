...
export default function SessionManager({
  ...
  const formatTimeRemaining = useCallback((milliseconds: number | null): string => {
  ...
  const getStatusColor = (time: number | null) => {
  ...
  // Get status icon
  const getStatusIcon = (time: number | null) => {
  ...
  return (
    <>
      ...
      {showInHeader && (
        <div className="flex items-center space-x-2 text-sm">
          ...
          {isSessionExpiringSoon() && (
            ...
            >
              <RefreshCw 
                className={`w-4 h-4 text-blue-600 ${isExtending ? 'animate-spin' : ''}`} 
              />
            </button>
          )}
        </div>
      ...
      {showWarningDialog && showWarningModal && (
