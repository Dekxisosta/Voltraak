...
import { mockLowStockAlerts } from './mocks'
...
export default function LowStockPage() {
  ...
  const columns = [
    ...
    { key: 'sku', label: 'SKU' },
    { key: 'current_stock', label: 'Current', render: (val, row) => <span className={val <= row.minimum_stock ? 'text-red-600 font-bold' : 'font-medium'}>{val}</span> },
    { key: 'reorder_point', label: 'Reorder Point' },
    { key: 'days_until_stockout', label: 'Days to Stockout', render: (val) => <span className={`font-bold ${val === 0 ? 'text-red-600' : val <= 7 ? 'text-yellow-600' : 'text-gray-700'}`}>{val === 0 ? 'NOW' : `${val} days`}</span> },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card><Card.Body><div className="text-center"><p className="text-3xl font-bold text-red-600">{data.alerts.filter(a => a.status === 'out_of_stock').length}</p><p className="text-sm text-gray-600">Out of Stock</p></div></Card.Body></Card>
        <Card><Card.Body><div className="text-center"><p className="text-3xl font-bold text-yellow-600">{data.alerts.filter(a => a.status === 'critical').length}</p><p className="text-sm text-gray-600">Critical</p></div></Card.Body></Card>
        <Card><Card.Body><div className="text-center"><p className="text-3xl font-bold text-orange-600">{data.alerts.filter(a => a.status === 'low').length}</p><p className="text-sm text-gray-600">Low Stock</p></div></Card.Body></Card>
        <Card><Card.Body><div className="text-center"><p className="text-3xl font-bold text-blue-600">{data.alerts.reduce((s, a) => s + a.suggested_order_qty, 0)}</p><p className="text-sm text-gray-600">Total Units to Order</p></div></Card.Body></Card>
      </div>
      ...
      <Card>
        <Card.Body>
          ...
          <Table data={filteredAlerts} columns={columns} emptyMessage="No low stock alerts" />
        </Card.Body>
      </Card>
    </div>
  )
