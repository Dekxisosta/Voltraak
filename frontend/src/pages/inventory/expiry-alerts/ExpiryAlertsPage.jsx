...
export default function ExpiryAlertsPage() {
  ...
  // Prioritize confirm modal state
  const [prioritizeTarget, setPrioritizeTarget] = useState(null)
    ...
    try {
      ...
      setData(prev => ({
        ...
        batches: prev.batches.map(b =>
          b.id === batch.id ? { ...b, status: 'written_off' } : b
        )
      }))
      addNotification({
        type: 'success',
        title: 'Batch Written Off',
        message: `${batch.batch_number} (${batch.quantity} units) has been written off`
      })
  ...
  const handleConfirmPrioritize = async () => {
    ...
    try {
      ...
      setData(prev => ({
        ...prev,
        batches: prev.batches.map(b =>
          b.id === batch.id ? { ...b, prioritized: true } : b
        )
      }))
      addNotification({
        type: 'success',
        title: 'Batch Prioritized',
        message: `${batch.batch_number} flagged for priority FEFO picking`
      })
  ...
  const columns = [
    ...
    { key: 'expiry_date', label: 'Expiry Date', sortable: true },
    {
      ...
      render: (val, row) => getExpiryBadge(row.status, val)
    },
    {
      key: 'actions',
      label: 'Actions',
      render: (_, row) => {
        ...
        if (row.status === 'expired') {
          return (
            <Button size="sm" variant="danger" icon={Trash2} onClick={() => setWriteOffTarget(row)}>
              Write Off
            </Button>
          )
        }
          ...
          return row.prioritized
            ? (
              <span className="inline-flex items-center gap-1 text-xs font-medium text-amber-600 dark:text-amber-400">
                <Check className="h-3.5 w-3.5" /> Prioritized
              </span>
            )
            : (
              <Button size="sm" variant="warning" icon={Flag} onClick={() => setPrioritizeTarget(row)}>
                Prioritize
              </Button>
            )
        }
  ...
  if (data.loading) {
    return (
      <div className="flex items-center justify-center min-h-96">
        <LoadingSpinner size="lg" message="Loading expiry data..." />
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Prioritize Confirm Modal */}
      <ConfirmModal
        isOpen={Boolean(prioritizeTarget)}
        onClose={() => { if (!prioritizeLoading) setPrioritizeTarget(null) }}
        onConfirm={handleConfirmPrioritize}
        title="Prioritize Batch for FEFO"
        message={prioritizeTarget
          ? `Flag ${prioritizeTarget.batch_number} (${prioritizeTarget.product_name}) for priority FEFO picking? This ensures pickers pull this batch first before newer stock.`
          : ''}
