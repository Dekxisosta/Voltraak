...
import type { TableColumn, PickingTask } from '@/types'

interface PickingPageData {
  pickingTasks: PickingTask[]
  loading: boolean
}

export default function PickingPage() {
  const [data, setData] = useState<PickingPageData>({
    pickingTasks: [],
    loading: true
  })
  ...
  const loadPickingTasks = async () => {
    try {
      ...
      const mockData: PickingTask[] = [
        {
          id: 1,
          order_number: 'ORD-2024-001',
          customer_name: 'Metro Manila Store',
          route: 'Route A',
          priority: 'high',
          items: [
            {
        ...
        },
        {
          ...
          due_time: '2024-08-12T16:00:00Z'
        }
      ]

      setTimeout(() => {
        setData({
          pickingTasks: mockData,
          loading: false
        })
      }, 800)
    } catch (error) {
  ...
  const handleMarkPicked = async (taskId: number, itemId: number) => {
    try {
      setData(prev => ({
        ...prev,
  ...
  const handleCompleteOrder = async (taskId: number) => {
    try {
      setData(prev => ({
        ...prev,
        pickingTasks: prev.pickingTasks.map(task => 
      ...
      addNotification({
        ...
        message: 'Picking task marked as complete'
      })
    } catch (error) {
  ...
  const columns: TableColumn<PickingTask>[] = [
    {
      key: 'order_number',
      label: 'Order Number',
      sortable: true,
