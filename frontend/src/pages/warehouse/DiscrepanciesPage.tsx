...
import { PageHeader, Card, Table, StatusBadge, Button, Input, Modal, SearchBar, LoadingSpinner } from '@/components/common'
import { useNotifications } from '@/hooks/useNotifications'
import type { TableColumn, DiscrepancyReport } from '@/types'

interface DiscrepanciesPageData {
...
export default function DiscrepanciesPage() {
  const [data, setData] = useState<DiscrepanciesPageData>({
    discrepancies: [],
    loading: true
  })
  ...
  const [newDiscrepancyForm, setNewDiscrepancyForm] = useState<NewDiscrepancyForm>({
    product_id: '',
    expected_quantity: 0,
    actual_quantity: 0,
  ...
  const loadDiscrepancies = async () => {
    try {
      ...
      const mockData: DiscrepancyReport[] = [
        {
          id: 1,
          report_number: 'DISC-2024-001',
          ...
          status: 'open',
          priority: 'high',
          notes: 'Possible theft or damage during transport',
          created_at: '2024-08-12T09:00:00Z',
          resolved_at: null
        },
        {
          id: 2,
          report_number: 'DISC-2024-002',
          ...
          priority: 'medium',
          notes: 'Extra unit found during physical count',
          created_at: '2024-08-11T14:30:00Z',
          resolved_at: null
        },
        {
          id: 3,
          report_number: 'DISC-2024-003',
          ...
          status: 'resolved',
          priority: 'high',
          notes: 'Units damaged during receiving - insurance claim filed',
          created_at: '2024-08-10T11:15:00Z',
          resolved_at: '2024-08-11T16:45:00Z'
        }
      ]

      setTimeout(() => {
        setData({
          discrepancies: mockData,
          loading: false
        })
      }, 800)
    } catch (error) {
  ...
  const handleSubmitDiscrepancy = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newDiscrepancy: DiscrepancyReport = {
        id: Date.now(),
        report_number: `DISC-2024-${String(data.discrepancies.length + 1).padStart(3, '0')}`,
  ...
  const columns: TableColumn<DiscrepancyReport>[] = [
    {
      key: 'report_number',
      label: 'Report #',
      sortable: true,
