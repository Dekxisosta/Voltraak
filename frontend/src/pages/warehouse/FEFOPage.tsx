...
import type { TableColumn, BatchRecommendation } from '@/types'

interface FEFOPageData {
  recommendations: BatchRecommendation[]
  loading: boolean
}

export default function FEFOPage() {
  const [data, setData] = useState<FEFOPageData>({
    recommendations: [],
    loading: true
  })
  ...
  const loadRecommendations = async () => {
    try {
      ...
      const mockData: BatchRecommendation[] = [
        {
          id: 1,
          batch_number: 'WM-B001',
          ...
          cost_per_unit: 25000,
          total_value: 125000,
          recommended_action: 'immediate_sale',
          notes: 'Contact sales team for promotional pricing'
        },
        {
          id: 2,
          batch_number: 'RF-B003',
          ...
          cost_per_unit: 35000,
          total_value: 105000,
          recommended_action: 'priority_sale',
          notes: 'Monitor closely, prepare for sale'
        },
        {
          id: 3,
          batch_number: 'AC-B002',
          ...
          urgency_level: 'safe',
          bin_location: 'C-01-03',
          supplier: 'Climate Solutions',
          cost_per_unit: 42500,
          total_value: 340000,
          recommended_action: 'normal_rotation',
          notes: 'Standard inventory rotation'
        }
      ]
  ...
  const handleConfirmPickOrder = async (batchId: number) => {
    try {
      addNotification({
        type: 'success',
  ...
  const handleMarkForDisposal = async (batchId: number) => {
    try {
      addNotification({
        type: 'warning',
  ...
  const columns: TableColumn<BatchRecommendation>[] = [
    {
      key: 'batch_number',
      label: 'Batch Number',
      sortable: true,
    ...
    },
    {
      ...
      render: (value, row) => (
        <div>
          ...
          <div className="text-sm text-gray-500">{row.bin_location}</div>
        </div>
      )
    },
    {
        ...
        <Card>
          <Card.Header>
            <div className="flex items-center">
              ...
              <h3 className="text-lg font-medium text-red-800">Critical Expiry Alert</h3>
            </div>
          </Card.Header>
          <Card.Body>
                ...
                {data.recommendations
                  ...
                  .map(rec => (
                    <div key={rec.id} className="flex items-center justify-between p-4 bg-red-50 border border-red-200 rounded-lg">
                      <div>
                        <div className="font-medium text-red-900">{rec.product_name}</div>
                        ...
                        {rec.notes && (
                          <div className="text-sm text-red-600 mt-1">{rec.notes}</div>
                        )}
                      </div>
                      <div className="flex space-x-2">
                        <Button
